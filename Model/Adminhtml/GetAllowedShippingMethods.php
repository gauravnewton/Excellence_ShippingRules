...
class GetAllowedShippingMethods extends \Magento\Framework\View\Element\Template
{
    ...
    */
    ...
    {        
        $allCarriers = $this->shippingConfig
            ->getAllCarriers($this->storeManager->getStore());

        $shippingMethodsArray = array();
        foreach ($allCarriers as $shippigCode => $shippingModel) {
            $shippingTitle = $this->scopeConfig
                ->getValue('carriers/' . $shippigCode . '/title');
    ...
    /* 
        This will return list of allowed carriers 
        available for current store 
    */
    public function getActiveCarriers()
    {
        $activeCarriers = $this->shippingConfig
            ->getActiveCarriers($this->storeManager->getStore());
